...
export default function ScreenDecks({modifyFooter}){
    ...
    return (
        <div className="deckofcards">
            <FillDeck cards={cards} modifyFooter={(icon)=> modifyFooter(icon, cards.length)} />
        </div>
    )
}
...
function FillDeck({cards, modifyFooter}){
    ...
    return cards.map((card, id) => {
        ...
        return <Card text={(id + 1)} question={question} answer={answer} modifyFooter={(icon)=> modifyFooter(icon)} />
    })
}